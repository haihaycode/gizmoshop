  ...
  <div class="product-container px-4 md:p-4">
    <div class="flex flex-col md:flex-row space-y-6 md:space-y-0 md:space-x-6">
      <div class="w-full space-y-4">
        <h2 class="text-2xl font-semibold text-gray-800">{{ product?.productName ? product.productName : '...' }}
        </h2>
        ...
        <div class="flex items-center space-x-4 mt-2">
          <span class="text-lg md:text-2xl font-bold text-red-500">
            {{ formatCurrency(product?.productPrice * (1 - product?.discountProduct / 100)) }}
          </span>
        ...
        <div class="text-sm text-gray-600 space-y-2 mt-2">
          <p><strong>Danh mục:</strong> <span class="text-blue-500 cursor-pointer hover:underline">{{
            product?.productCategories?.name || 'No Category'
              }}</span>
          </p>
          <p><strong>Thương hiệu:</strong> <span class="text-blue-500 cursor-pointer hover:underline">{{
            product?.productBrand?.name ||
            'Unknown'
          }}</span></p>
          <p><strong>Số lượng còn lại:</strong> {{ product?.productInventoryResponse?.quantity
            || 'N/A' }}</p>
          <p><strong>Mô tả:</strong> {{ product?.productShortDescription?.slice(0,
            product.productShortDescription?.indexOf("[[[")) || product?.productShortDescription }}</p>

        </div>

        <div class="flex flex-col md:flex-row items-center space-y-1 md:space-y-0 md:space-x-4 mt-4">
          <div class="flex items-center border border-gray-300 rounded">
            <button @click="decreaseQuantity" class="px-3 py-1 md:px-4"><i class="bx bx-minus"></i></button>
            <input type="number" v-model="quantity" min="1"
              class="w-12 text-center border-l border-r border-gray-300" />
            <button @click="increaseQuantity" class="px-3 py-1 md:px-4"><i class="bx bx-plus"></i></button>
          </div>
          ...
          <button @click="toggleWishlist"
            ...
            <span class="hidden lg:inline ml-1">{{ isInWishlist ? "Đã Yêu Thích" : "Thêm vào Yêu Thích" }}</span>
            <span class="inline lg:hidden ml-1">{{ isInWishlist ? "Đã Yêu Thích" : "Thêm" }}</span>
          </button>
        </div>
      </div>
    </div>
